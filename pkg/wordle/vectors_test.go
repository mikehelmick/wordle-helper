...
type solveCase struct {
	Name string `json:"name"`
	// Dictionary is omitted when the case runs against the full word list,
	// which both implementations ship identically.
...
func generateVectors() (*vectorFile, error) {
	out := &vectorFile{
		Note: "Generated by `go test ./pkg/wordle -run TestVectors -update`. " +
			...
			"A solve case with no `dictionary` runs against the full word list.",
		WordLen:  WordLen,
	...
	fullDict := DefaultDictionary()

	for _, in := range solveInputs {
		dict := fullDict
		if in.Dictionary != nil {
			fixtureDict, err := NewDictionary(in.Dictionary)
			if err != nil {
				return nil, err
			}
			dict = fixtureDict
